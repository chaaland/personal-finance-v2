<script lang="ts">
  ...
  const subtextValue = $derived(
    `${formatCurrency(currentValue)} / ${formatCurrency(targetValue)}`
  );

  const runwayText = $derived(
    `${runwayYears.toFixed(1)} years runway at ${formatCurrency(projectedSpend)}/yr spend`
  );
...
<div class="card" id={cardId}>
  ...
  >
    <div class="content">
      ...
      <p class="value">{progressPct.toFixed(0)}%</p>
      <p class="subtext">{subtextValue}</p>
    </div>
