...
def get_combined_networth(data: FinanceData) -> pl.DataFrame:
    ...
    """
    us_df = data.us_networth.select(
        pl.col("Dates"),
        (pl.col("Net") * pl.col("Conversion")).alias("US_USD"),
    ).sort("Dates")

    uk_df = data.uk_networth.select(
        pl.col("Dates"),
        (pl.col("Net") * pl.col("Conversion")).alias("UK_USD"),
    ).sort("Dates")

    # Get all unique dates from both datasets
    all_dates_df = pl.concat([us_df.select("Dates"), uk_df.select("Dates")]).unique().sort("Dates")
    ...
    combined_df = all_dates_df.join_asof(us_df, on="Dates", strategy="backward").join_asof(
        uk_df, on="Dates", strategy="backward"
    )
    ...
    # Calculate total
    combined_df = combined_df.with_columns((pl.col("US_USD") + pl.col("UK_USD")).alias("Total_USD"))
...
def get_current_networth(data: FinanceData) -> Decimal:
    ...
    return combined_df.select("Total_USD").row(-1)[0]
...
def get_ytd_networth_change(data: FinanceData) -> tuple[Decimal, Decimal]:
    ...
    if len(year_data_df) == 1:
        ...
        if prev_year_data_df.is_empty():
            return Decimal("0"), Decimal("0")
        start_value = prev_year_data_df.select("Total_USD").row(-1)[0]  # Last value of previous year
    else:
        start_value = year_data_df.select("Total_USD").row(0)[0]  # First value of current year
...
def get_yoy_networth_changes(data: FinanceData) -> pl.DataFrame:
    ...
    # Add year column
    combined_df = combined_df.with_columns(pl.col("Dates").dt.year().alias("Year"))

    # Get first and last value per year
    yearly_df = combined_df.group_by("Year").agg(
